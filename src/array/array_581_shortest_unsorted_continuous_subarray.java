...
class array_581_shortest_unsorted_continuous_subarray {
    ...
    /**
     * 最佳实践思路：这个写法和onepass有异曲同工的感觉，一路下去，获得目前的最大值和最小值，然后将现在的值和暂时的最大值最小值对比
     * 如果当前值比获得的最大值还小的话，这样终点指针挪到该位置，原理是如果当前不需要排序的话，当前值其实理应比最大值大
     * 同理，最小值也一样，如果当前值大于最小值的话，起点的指针挪到这个值，代表这之前的值已经有序了
     */
}
